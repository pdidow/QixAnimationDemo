...
namespace QixAnimationDemo
{
    public class Game1 : Game
    {
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            {
                Vector2 start = new Vector2(x1(t), y1(t));
                Vector2 end = new Vector2(x2(t), y2(t));
        ...
        float x1(float t)
        {
            return (float)(Math.Sin(t / 10) * 100 + Math.Sin(t / 5) * 20);
        }

        float y1(float t)
        {
            return (float)(Math.Cos(t / 10) * 100);
        }

        float x2(float t)
        {
            return (float)(Math.Sin(t / 10) * 200 + Math.Sin(t) * 2);
        }

        float y2(float t)
        {
            return (float)(Math.Cos(t / 20) * 200 + Math.Cos(t / 12) * 20);
        }
    }
}